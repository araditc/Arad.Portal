...
public static class CaptchaHelper
{
    public static FileContentResult GenerateCaptchaImage(this ISession session, int lifeTime, bool noisy = true)
    {
        ...
        Image<Rgba32> img = new Image<Rgba32>(130, 30);
        img.Mutate(c => c.BackgroundColor(Color.White));

        Font font = SystemFonts.CreateFont("Tahoma", 17, FontStyle.Italic);
        PointF location = new PointF(10, 5);
        img.Mutate(c => c.DrawText(captcha, font, Color.DimGray, location));
        ...
        if (noisy)
        {
            ...
            for (i = 1; i < 10; i++)
            {
                ...
                x = rand.Next(0, 130);
                y = rand.Next(0, 30);
                IPath polygon = new EllipsePolygon(new(x - 1, y - 1), r);
        ...
        fileResult = new(byteArray, "image/Jpeg");
        img.Dispose();

        CaptchaModel captModel = new CaptchaModel()
                                 {
                                     Code = (a + b).ToString(),
                                     ExpirationDate = DateTime.Now.AddMinutes(lifeTime)
                                 };

        string serializedData = JsonConvert.SerializeObject(captModel);
    ...
    public static bool ValidateCaptcha(this ISession session, string code)
    {
        ...
        string data = session.GetString("SystemCaptcha");
        ...
        try
        {
            CaptchaModel capt = JsonConvert.DeserializeObject<CaptchaModel>(data);
