...
namespace Arad.Portal.UI.Shop.Dashboard.Helpers
{
    ...
    public static class CaptchaHelper
    {
        public static FileContentResult GenerateCaptchaImage(this ISession session, int lifeTime, bool noisy = true)
        {
            ...
            using (SixLabors.ImageSharp.Image img = new Image<Rgba32>(130, 30))
            {
                img.Mutate(_ => _.BackgroundColor(Color.White));
                var font = SystemFonts.CreateFont("Tahoma", 10, FontStyle.Regular);
                var location = new PointF(10, 15);
                img.Mutate(ctx => ctx.DrawText(captcha, font, Color.Black, location));
            }

            return fileResult;
        }
        //public static FileContentResult aaaa(this ISession session, int lifeTime, bool noisy = true)
        //{
            //var rand = new Random((int)DateTime.Now.Ticks);
            ////generate new question
            //int a = rand.Next(10, 99);
