...
public static class DateHelper
{
    ...
    public static string GetEnglishDatesForPersianStartAndEndDatesOfCurrentMonth()
    {
        ...
        PersianCalendar newDateObject = new PersianCalendar();
        int persianDayOfMonth = newDateObject.GetDayOfMonth(date);
        int thisMonth = newDateObject.GetMonth(date);
        int thisYear = newDateObject.GetYear(date);
        DateTime startDate = date.Subtract(new TimeSpan(persianDayOfMonth - 1, 0, 0, 0));
        int daysOfThisMonth = newDateObject.GetDaysInMonth(thisYear, thisMonth);
        DateTime endDate = date.AddDays(daysOfThisMonth - persianDayOfMonth);
    ...
    public static string ToPersianVeryFullDateTime(this DateTime d)
    {
        ...
        d = d.ToLocalTime();
        PersianCalendar newDateObject = new PersianCalendar();
        string convertedDate = newDateObject.GetYear(d).ToString("0000") + "/" +
                               newDateObject.GetMonth(d).ToString("00") + "/" +
                               newDateObject.GetDayOfMonth(d).ToString("00");
        string hour = newDateObject.GetHour(d).ToString("00");
        string minutes = newDateObject.GetMinute(d).ToString("00");
        string amPm = int.Parse(hour) >= 0 && int.Parse(hour) < 12 ? "ق.ظ" : "ب.ظ";
    ...
    public static string ToPersianLetDateTime(this DateTime d)
    {
        ...
        PersianCalendar newDateObject = new PersianCalendar();
        string year = newDateObject.GetYear(d).ToString("0000");
        string month = newDateObject.GetMonth(d).ToString("00");
        string day = newDateObject.GetDayOfMonth(d).ToString("00");
        string hour = newDateObject.GetHour(d).ToString("00");
        string mins = newDateObject.GetMinute(d).ToString("00");
        if (day.StartsWith("0"))
        {
            day = day.Substring(1);
        }
        string dayOfWeek = newDateObject.GetDayOfWeek(d).ToString();
    ...
    public static string ToPersianLetDateTimeWithSeconds(this DateTime d)
    {
        ...
        string hour = newDateObject.GetHour(d).ToString("00");
        string mins = newDateObject.GetMinute(d).ToString("00");
        string secs = newDateObject.GetSecond(d).ToString("00");
    ...
    public static DateTime ToEnglishDate(this string d)
    {
        ...
        {
            d = d.ToEngishString();
            string[] daytttt = d.Split('/');
            int year = int.Parse(daytttt[0]);
            int month = int.Parse(daytttt[1]);
            int day = int.Parse(daytttt[2]);
    ...
    public static DateTime ToEnglishDateTime(this string d)
    {
        d = d.ToEngishString();
        string[] daytttt = d.Split('/');
        string[] dayy = daytttt[2].Split(' ');
        string[] times = dayy[1].Split(':');
        int hour = Convert.ToInt32(times[0]);
        int mintute = Convert.ToInt32(times[1]);
        int second = Convert.ToInt32(times[2]);
        int year = int.Parse(daytttt[0]);
        int month = int.Parse(daytttt[1]);
        int day = int.Parse(dayy[0]);
    ...
    public static string ToPersianDdateNoBackSlash(this DateTime d)
    {
        ...
        PersianCalendar newDateObject = new PersianCalendar();
        string convertedDate = newDateObject.GetYear(d).ToString("00") +
                               newDateObject.GetMonth(d).ToString("00") +
                               newDateObject.GetDayOfMonth(d).ToString("00");
